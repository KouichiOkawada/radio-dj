...
- 全ニュース記事は、RSSの出典・見出し・概要・配信時刻を `NewsBulletin` でOpenAIへ渡し、30〜60秒の自然なラジオ原稿へ要約してからTTS化する。AI失敗時だけ、無音回避のため従来の事実限定RSS原稿へフォールバックする。
- News Continuousは必ず `AI要約1記事 → 同じ記事への約3分の私見・解説 → 次の記事`。定時ラジオ枠は複数見出しと短いコメントを維持する。
- ニュース要約、AIコメント、曲間DJを「発話」として扱い、各時刻の最初の発話直前に時刻案内を挿入する。音楽の途中では割り込まない。たとえば音楽中に12時を跨ぐと、曲終了後の最初の発話冒頭で「正午を回りました。現在時刻は12時です」と告げる。
- 時刻音声もバックグラウンド先読みし、TTS待ちで音楽を止めない。UIはニュース中を `AI SUMMARY`、記事へのDJ私見中を `NEWS COMMENTARY` と区別し、後者では記事カードとDJアバターを同時に維持する。
- OpenAI費用制御：Icecastの実リスナー数を3秒ごとに監視し、未確認または0人なら全OpenAI互換HTTP呼び出しを直前ガードで停止する。ニュースAI要約/解説の新規生成も休止し、音楽だけは継続。1人以上になるとニュースpreloaderを即wakeして自動再開する。UI状態は `リスナー待機中 · OpenAI停止`。管理APIがない外部Icecastでは公開 `status-json.xsl` をmount指定で読む。
